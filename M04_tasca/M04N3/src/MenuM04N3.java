		...
		do {
			...
			try {
				...
				if (Arrays.asList(plats).contains(plato)) {
					...
					// Lanzamos la Exception que hemos creado
					throw new PlatoErroneo("Este plato no está disponible en nuestro menú"); // que hemos creado
				}
			} catch (PlatoErroneo e) {
			...
			while (opcion != 1 && opcion != 0) {
				try {
					...
					if (opcion == 1) {
						...
						// Lanzamos la Exception que hemos creado.
						throw new Siono("Lo siento. Debe introducir la opcion 1 o 0"); // hemos creado
					}
